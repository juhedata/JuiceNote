<template>
  <div>
    <div class="relative">
      <!-- 语言 -->
      <span v-html="lan"
        class="absolute right-0 top-0 z-20 rounded-bl-lg bg-cyan-800/20 px-2 py-1 text-sm text-info"></span>

      <!-- 运行按钮 -->
      <button contenteditable="false" class="btn-sm btn absolute bottom-8 right-2 z-20 transition-none"
        :class="{ loading: running }" @click="handleRun" v-html="runTitle" v-show="runnable"></button>
      ...
      <div ref="codeDom" class="relative z-10">
        ...
        <div class="code-block-operators absolute bottom-0" contenteditable="false" v-if="editable">
          ...
          <div class="flex justify-end">
            <button v-bind:data-clipboard-text="codeForCopy"
              class="copy justify-end self-end justify-self-end">复制代码</button>
            <button @click="handleToggleRun" v-html="runnable ? '关运行' : '开运行'"
              class="justify-end self-end justify-self-end"></button>
